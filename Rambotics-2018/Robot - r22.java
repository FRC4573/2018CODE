...
public class Robot extends TimedRobot {
	...
	public void robotInit() {
		...
		seqState = 'E';
		autoSeq = "E0";
		SmartDashboard.putString("DB/String 5", "C");
	...
	public void autonomousInit() {
		...
		autoMap = SmartDashboard.getString("DB/String 5", "C");  // Default = Center if none selected
		...
		switch(autoMap) {
			case "LL":{
				...
				if (autoProg == 1) {
					autoSeq = "W0D6R2D1E0";  // Tested - Drop on front tip of the switch
				}
				if (autoProg == 2) {
					autoSeq = "W0D4W9W5E0";// For Prog 2 - Just break Autoline
				}
				break;
			}
			case "LR":{
				//autoSeq = "W0D2R2D9D3H1L2D4L2D1E0";// calculated
				autoSeq = "W0H1R2D7H1L2D3E0";// Tested
				if (autoProg == 2) {
					autoSeq = "W0D4W9W5E0";// For Prog 2 - Just break the autoline and wait
				}
				break;
			}
			case "CL":{
				//autoSeq = "W0D2L2D5H1R2D4R2D1E0"; // Calculated
				autoSeq = "W0D1L1D4R1H1E0"; // Tested
				break;
			}
			case "CR":{
				autoSeq = "W0D1R1D3L1D1H1E0"; // Tested
				break;
			}
			case "RL":{
				//autoSeq = "W0D2L2D9D3H1R2D4R2D1E0"; //Opposite of LR
				autoSeq = "W0H1L2D7H1R2D3E0"; //Tested
				if (autoProg == 2) {
					autoSeq = "W0D4W9W5E0";// For Prog 2 - Just break the autoline and wait
				}
				break;
			...
			case "RR":{
				...
				if (autoProg == 1) {
					autoSeq = "W0D6L2D1E0";  // Tested - Drop on front tip of the switch
				}
				if (autoProg == 2) {
					autoSeq = "W0D4W9W5E0";// For Prog 2 - Just break Autoline
				}
		...
		public void updateDisplays() {
			if (displayCtr % 25 == 0){	// Update displays on Dashboard every ~500msec
	    	    ...
	    	    //SmartDashboard.putString("DB/String 7", "");
	    	   	SmartDashboard.putString("DB/String 8", Double.toString(Math.round(gyro.getAngle() * 1d)/1d ) + " Deg.");  // round to 0 decimal places
	            SmartDashboard.putString("DB/String 9", Double.toString(autoProg));  // 
		 	}
			displayCtr++;
		}
		// ***************************************************************************
